...
async function createSticker(buffer, options = {}) {
  const { isVideo = false, pack = "", author = "RyzuBot" } = options
...
async function addExifMetadata(webpBuffer, packName, author) {
  ...
  try {
    ...
    console.log("[STICKER] ✅ node-webpmux berhasil, size:", result.length)
    return result
  } catch (e) {
    console.log("[STICKER] ⚠️ node-webpmux gagal:", e.message)
    console.log("[STICKER] 🔄 Pakai fallback manual RIFF...")
