...
module.exports = {
    ...
    execute: async ({ ryzu, from, sender, args, command, reply, funcs }) => {
        try {
            ...
            // PENTING: Pengecekan aman untuk args Array
            // Jika args kosong (misal cuma ngetik .ww), cmdArg akan jadi null
            const cmdArg = (args && args.length > 0 && typeof args === 'string') ? args.toLowerCase() : null;
            // Gabungkan argumen sisanya jadi nama target
            const targetArg = (args && args.length > 1) ? args.slice(1).join(" ") : "";

            // --- FITUR LEADERBOARD ---
            if (cmdArg === "leaderboard" || cmdArg === "lb") {
                let lbType = (args && args.length > 1) ? args : "total_wins";
